...
def fetch_and_validate_data(url, serializer):
    ...
    try:
        ...
        error_message = [
            ...
            for data, error in zip(fetched_data, e.detail) if error
        ]
...
def create_characters(characters_data):
    ...
    for character_data in characters_data:
        character = models.Character(**{field: character_data.get(field) for field in fields})
...
def create_starships(starships_data):
    ...
    starships_to_create = []
    starship_mappings = {}
    fields = ['name', 'model', 'cost_in_credits', 'hyperdrive_rating', 'created', 'swapi_url']
    for starship_data in starships_data:
        starship = models.Starship(**{field: starship_data.get(field) for field in fields})
...
def create_starship_character_relationships(starships_data, starship_mappings, character_mappings):
    """
...
def populate_database(films_data, characters_data, starships_data):
    ...
    create_starship_character_relationships(starships_data, starship_mappings, character_mappings)
